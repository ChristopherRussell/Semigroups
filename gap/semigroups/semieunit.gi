...
#           Add(gens, MTSE(S, c[1], RepAct(c[j], c[1])));
#         od;
...
#       stabs := [];
#       found := [];
#       for u in c do # This section doesn't work as intended
...
#           Add(gens, MTSE(S, orbs[1][1], RepAct(orb[1], orbs[1][1])));
#           check := true;
#         od;
#       fi;
#
...
SEMIGROUPS.MTSSmallGen := function(S)
  local G, Sl, X_Y, Y_X, comps, lookup, RepAct, _Stab, _Orb, hom, act, gens,
  acts, po, top, sl, above, c, stab, check, orbs, stabs, found, Gcj, j, nbrs, r,
  i, cat, pos, combined, new_orbs, g, n, sizes, Gc1, alpha, gs, u, nbr, v, o,
  orb;
...
function(filt, S)
  local Es, iso_pg, map, G, xx, M, iso, yy, ids, cong, grp, hom, map_G, Dcl, n,
  cosets, x, xiny, yinx, D, s, R, e, Ge, h, y_pos, x_pos, act, ah, edgy, act2,
  i, H;
  ...
  if Size(Es) > 1 then
    ...
    G      := Group(List(SmallGeneratingSet(G)));
    map    := MappingByFunction(Range(iso_pg), G, map);
    xx     := Digraph([[1]]);
    M      := McAlisterTripleSemigroup(G, xx, xx);
