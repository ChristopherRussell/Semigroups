...
###############################################################################
###############################################################################
FindRZMSTripleParametersByOrder := function(k)
  local out, d, e;
  out := [];
...
RZMSMatricesByShape := function(nr_rows, nr_cols, G, shape)
  ...
  # small) so we have RZMSMatricesByShapeEasyCase some of these situations.
...
RZMSMatrices := function(order)
  ...
  out := [];
  parameters := FindRZMSTripleParametersByOrder(order);
